...
## Backend
- [ ] **Config binding mismatch**: `application.yml` uses `max-xpath-length` but `CatalogProperties.java` expects `maxFieldPathLength`. Validation limits may silently use defaults instead of configured values.
