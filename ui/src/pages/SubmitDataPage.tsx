...
import MetadataEditorModal from '../components/upload/MetadataEditorModal';
import type { UploadBin } from '../types';
...
const SubmitDataPage: React.FC = () => {
  ...
  const handleFileDrop = async (e: React.DragEvent) => {
    ...
    if (files.length > MAX_FILES) {
      toast.error(`Maximum ${MAX_FILES} files allowed per upload. You selected ${files.length}.`);
      return;
    }

    if (files.length > 0 && selectedContext) {
      const validCount = await scanFiles(
        files,
        selectedContext.metadataRules || {},
  ...
  // Calculate totals from the new bin structure
  const totalFiles = bins.reduce((sum, bin) => sum + bin.files.length, 0);
  const totalFields = bins.reduce((sum, bin) =>
    sum + bin.files.reduce((s, f) => s + f.fieldCount, 0), 0);
  const totalAttributes = bins.reduce((sum, bin) =>
    sum + bin.files.reduce((s, f) => s + f.attributeCount, 0), 0);
  ...
  return (
    <Layout>
              ...
              <div>
                <div className="mb-3 flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-slate-500 text-xs">Upload up to {MAX_FILES} XML files to extract field observations</p>
                  </div>
                </div>
      ...
      {editingBin && selectedContext && (
        <MetadataEditorModal
          bin={editingBin}
          context={selectedContext}
          onSave={(updatedFiles) => {
            updateBinFiles(editingBin.id, updatedFiles);
            setEditingBin(null);
          }}
