...
| ID | Requirement | Acceptance Criteria |
|----|-------------|---------------------|
| REQ-3.1 | Results table | Display results in table with fixed columns: fieldPath, context, minOccurs, maxOccurs, allowsNull, allowsEmpty. All columns sortable. Null? and Empty? columns include filter dropdowns in headers (All/Yes/No). **No metadata columns** - metadata is shown in detail panel only (scales to any number of contexts). |
...
| REQ-3.3 | Client-side filtering | Left sidebar provides instant client-side filters: faceted metadata filtering (REQ-3.8) and text filter on path. Null?/Empty? filters are in table column headers (REQ-3.1). All filters apply instantly without API calls. |
